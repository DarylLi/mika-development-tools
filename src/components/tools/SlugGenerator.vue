<template>
  <div class="single-tool">
    <h2><i class="fas fa-link"></i> {{ $t('tools.slugGenerator.ui.title') }}</h2>
    
    <div class="example-section">
      <button class="example-btn" @click="loadExample">
        <i class="fas fa-lightbulb"></i> {{ $t('tools.slugGenerator.ui.loadExample') }}
      </button>
    </div>
    ...
    <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; margin-bottom: 1rem;">
      <button @click="generateSlug('kebab')">{{ $t('tools.slugGenerator.ui.kebabCase') }}</button>
      <button @click="generateSlug('snake')">{{ $t('tools.slugGenerator.ui.snakeCase') }}</button>
      <button @click="generateSlug('camel')">{{ $t('tools.slugGenerator.ui.camelCase') }}</button>
      <button @click="generateSlug('pascal')">{{ $t('tools.slugGenerator.ui.pascalCase') }}</button>
    </div>
